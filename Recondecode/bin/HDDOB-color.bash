...
####  Put the parts together for airmet at a higher elevations...view using upper air menu
  ...
  if [ $acarsextrappressure == '//' ]; then
     ...
     originalplane=${plane} 
      windef="000" 
     echo "------------------------- ${ssmr}      ----------" 
     if [ `echo "${ssmr} > 34" | bc` -eq 1 ];then
                winddef="34"  
                plane="${originalplane}TS"
                echo "inside34"   
      fi

     if [ `echo "${ssmr} > 64" | bc` -eq 1 ];then
                winddef="64"
                plane="${originalplane}HU"
                echo "inside64" 
        fi
 
    
     if [ `echo "${ssmr} > 96" | bc` -eq 1 ];then
                winddef="96"
               plane="${originalplane}MA"  
              echo "inside 96"          
        fi
  
        if [ `echo "${ssmr} > 200" | bc` -eq 1 ];then
                winddef="000"
        fi
     ...
     if [ `echo "${ssmr} > 34" | bc` -eq 1   ]; then
         ...
         sed -e "s/plane/${plane}/g" -e "s/date/${date}/g" -e "s/lat/${lat}/g" -e "s/lon/${lon}/g" -e "s/time/${loophour}${obminute}/g" \
          -e "s/level/${ssmr}/g" -e "s/temp/${sign}${acarsextrappressure}/g" -e "s/direction/${direction}/g" -e "s/speed/${speed}/g"  airep.template > ${workingdirectory}/data/UANT01_CWAO_acars${date}${lat}${lon}${obhour}${filelabel}${loophour}${plane}
         ...
         chmod 777 ${workingdirectory}/data/UANT01_CWAO_acars${date}${lat}${lon}${obhour}${filelabel}${loophour}${plane}
         cp ${workingdirectory}/data/UANT01_CWAO_acars${date}${lat}${lon}${obhour}${filelabel}${loophour}${plane} ${finaloutputdir}
